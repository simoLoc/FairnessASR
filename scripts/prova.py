...
def get_results_by_category(dataset, processor, model, category, model_name, device):
    ...
    if category == "overall":
        ...
        return [evaluate_by_group(dataset.select(range(num_samples_max)), processor, model, "Overall", model_name, device)]
        ...
        aave = dataset.filter(lambda x: x["aave"] == 1).select(range(num_samples_max))
        sae = dataset.filter(lambda x: x["sae"] == 1).select(range(num_samples_max))
        spanglish = dataset.filter(lambda x: x["spanglish"] == 1).select(range(num_samples_max))
        chicano = dataset.filter(lambda x: x["chicano_english"] == 1).select(range(num_samples_max))
        others = dataset.filter(lambda x: x["other_dialect_accent"] == 1).select(range(num_samples_max))
        ...
        aave_men = dataset.filter(lambda x: x["aave"] == 1 and x["men"] == 1 and x["women"] == 0).select(range(num_samples_max))
        aave_women = dataset.filter(lambda x: x["aave"] == 1 and x["women"] == 1 and x["men"] == 0).select(range(num_samples_max))

        sae_men = dataset.filter(lambda x: x["sae"] == 1 and x["men"] == 1 and x["women"] == 0).select(range(num_samples_max))
        sae_women = dataset.filter(lambda x: x["sae"] == 1 and x["women"] == 1 and x["men"] == 0).select(range(num_samples_max))

        spanglish_men = dataset.filter(lambda x: x["spanglish"] == 1 and x["men"] == 1 and x["women"] == 0).select(range(num_samples_max))
        spanglish_women = dataset.filter(lambda x: x["spanglish"] == 1 and x["women"] == 1 and x["men"] == 0).select(range(num_samples_max))

        chicano_men = dataset.filter(lambda x: x["chicano_english"] == 1 and x["men"] == 1 and x["women"] == 0).select(range(num_samples_max))
        chicano_women = dataset.filter(lambda x: x["chicano_english"] == 1 and x["women"] == 1 and x["men"] == 0).select(range(num_samples_max))

        other_men = dataset.filter(lambda x: x["other_dialect_accent"] == 1 and x["men"] == 1 and x["women"] == 0).select(range(num_samples_max))
        other_women = dataset.filter(lambda x: x["other_dialect_accent"] == 1 and x["women"] == 1 and x["men"] == 0).select(range(num_samples_max))
...
def main():
    ...
    model_list = ["facebook/wav2vec2-base-960h", "openai/whisper-medium"]
